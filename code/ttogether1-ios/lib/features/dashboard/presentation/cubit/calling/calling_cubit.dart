...
class CallingCubit extends Cubit<CallingState> {
  ...
  Widget checkLocalVideo(bool isLandscape) {
  ...
  Widget localView(bool isLanscape) {
    ...
    return isTurnOff
        ...
        : Container(
            ...
            child: Center(
              child: checkLocalVideo(isLanscape),
            ),
          );
  }

  Widget remoteView(int uid,
      {bool isInstructor = false, bool isVideoMuted = false}) {
    return isVideoMuted
        ? VideoOffSurface()
        : RtcRemoteView.SurfaceView(
            uid: uid,
            renderMode:
                (isInstructor) ? VideoRenderMode.Hidden : VideoRenderMode.Fit,
          );
  }
            ...
            if (myUserNumber != null &&
              ...
              list[i] = isGroupView
                  ? remoteView(uid,
                      ...
                      isVideoMuted: remoteStreamInfo.isVideoMuted)
      ...
      remoteUsers.forEach((element) {
        if (element.tshUser.userInfo.userNumber ==
          ...
          list[0] = remoteView(element.tshUser.userInfo.userNumber,
              isInstructor: true, isVideoMuted: element.isVideoMuted);
          ...
          list[1] = remoteView(
            ...
            isVideoMuted: element.isVideoMuted,
          );
  ...
  Future<void> initialize() async {
    ...
    await engine.setParameters("{\"che.video.keep_prerotation\":false}");
    await engine.setVideoEncoderConfiguration(VideoEncoderConfiguration(
        // dimensions: VideoDimensions(width: 1280, height: 720),
        dimensions: VideoDimensions(width: 850, height: 480),
