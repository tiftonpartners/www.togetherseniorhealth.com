...
export class GlobalEventService {
  ...
  constructor(
    ...
    this.listenTo(this.sessionStateService.stateChanged$);
    this.listenTo(this.auth0Service.userProfile$);
  }

  // Receive events from Components or Services. They are
  // sent to the server, and forwarded to other listeners
  // of global events
  listenTo = (obser$: Observable<GlobalEvent>) => {
    obser$.subscribe((event: GlobalEvent) => {
      try {
        ...
        if (event.subject != SERVER) this.newCommandSource$.next(event);
      } catch (e) {
