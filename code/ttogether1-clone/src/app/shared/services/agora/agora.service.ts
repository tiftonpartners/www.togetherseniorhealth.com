...
import AgoraRTC, { IAgoraRTCClient, LowStreamParameter, RemoteStreamType, UID } from 'agora-rtc-sdk-ng';
...
export class AgoraService {
      ...
      this._onRemoteUserStateEvent.emit({
        ...
        user: {
          ...
          audioTrack: {
            ...
            getVolumeLevel: () => {
              if (user.audioTrack) {
                return <number>user.audioTrack.getVolumeLevel();
              }
            },
            play: () => {
              if (user.audioTrack) {
            ...
            getMediaStream: () => {
              if (user.audioTrack) {
                return new MediaStream([<MediaStreamTrack>user.audioTrack.getMediaStreamTrack()]);
              }
            },
            getStats: () => {
              if (user.audioTrack) {
          ...
          videoTrack: {
            ...
            getMediaStream: () => {
              if (user.videoTrack) {
                return new MediaStream([<MediaStreamTrack>user.videoTrack.getMediaStreamTrack()]);
              }
            },
            getStats: () => {
              if (user.videoTrack) {
  ...
  public join(channelName: string, token: string, uid?: number): IJoinChannel<IMediaTrack> {
    let joinChannel = new JoinChannel(this.client, this.config, channelName, token, uid);
  ...
  public joinAudioFile(): IAudioFileJoinChannel<IAudioFileTrack> {
