...
import { CredentialsService, Auth0Credentials } from './credentials.service';
  ...
  describe('login', () => {
    ...
    it('should authenticate user', fakeAsync(() => {
      ...
      request.subscribe(() => {
        ...
        expect((credentialsService.credentials as Auth0Credentials).token).toBeDefined();
        expect((credentialsService.credentials as Auth0Credentials).token).not.toBeNull();
      });
    }));
